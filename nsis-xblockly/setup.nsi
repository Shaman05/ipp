...
; 安装程序初始定义常量
!define PRODUCT_NAME "XBlockly"
!define SHORTCUT_NAME "XBlockly"
!define PRODUCT_VERSION "1.0.13.0"
!define PRODUCT_PUBLISHER "每创电子信息科技股份有限公司"
!define PRODUCT_WEB_SITE "http://www.mylecon.com"
!define PRODUCT_DIR_REGKEY "Software\Microsoft\Windows\CurrentVersion\App Paths\XBlockly.exe"
!define PRODUCT_UNINST_KEY "Software\Microsoft\Windows\CurrentVersion\Uninstall\${PRODUCT_NAME}"
!define PRODUCT_UNINST_ROOT_KEY "HKLM"
!define PRODUCT_DEFAULT_PATH "C:\mylecon\XBlockly"
...
BrandingText "　每创科技 www.mylecon.com"
...
Function AlterButton
   StrCpy $R9 1
   Call RelXBlocklyPage
   Abort
FunctionEnd

Function RelXBlocklyPage
...
Function onLink
  ExecShell open "https://www.mylecon.com/common/license-agreement"
FunctionEnd
...
Section Main
  ...
  SetOverwrite ON 
  File "..\XBlockly.exe"
  File "..\views_resources_200_percent.pak"
  File "..\version"
  File "..\ui_resources_200_percent.pak"
  File "..\snapshot_blob.bin"
  File "..\pdf_viewer_resources.pak"
  File "..\node.dll"
  File "..\natives_blob.bin"
  ...
  File "..\blink_image_resources_200_percent.pak"
  #File "..\uninst.exe"
  ...
  #		CreateShortCut "$DESKTOP\${SHORTCUT_NAME}.lnk" "$INSTDIR\XBlockly.exe" ""
  #${EndIf}
  
  ${If} $Checkbox2_State == 1
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\1_${PRODUCT_NAME}.lnk" "$INSTDIR\XBlockly.exe" "XBlockly"
  ...
  ${If} $Checkbox3_State == 1
  	${if} $R0 >= 6.0
	    SetOutPath "$INSTDIR"
		#CreateShortCut "$INSTDIR\${SHORTCUT_NAME}.lnk" "$INSTDIR\XBlockly.exe"
		#SetOutPath $WINDIR
	 ...
	 ${else}
		  Delete "$DESKTOP\${SHORTCUT_NAME}.lnk"
		  CreateShortCut "$DESKTOP\${SHORTCUT_NAME}.lnk" "$INSTDIR\XBlockly.exe"
		  CreateShortCut "$QUICKLAUNCH\${PRODUCT_NAME}.lnk" "$INSTDIR\XBlockly.exe"
	  ${EndIf}
  ${EndIf}
  ...
  WriteRegStr HKLM "${PRODUCT_DIR_REGKEY}" "" "$INSTDIR\XBlockly.exe"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayName" "$(^Name)"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "UninstallString" "$INSTDIR\uninst.exe"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\XBlockly.exe"
...
Function un.onUninstSuccess
  ...
  ExecShell "open" "http://www.mylecon.com/"
FunctionEnd
