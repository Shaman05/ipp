...
!define PRODUCT_VERSION "2.1.3.6"
!define PRODUCT_PUBLISHER "东方财富信息股份有限公司"
!define PRODUCT_WEB_SITE "http://www.eastmoney.com"
!define PRODUCT_DIR_REGKEY "Software\Microsoft\Windows\CurrentVersion\App Paths\EMTrade.exe"
!define PRODUCT_UNINST_KEY "Software\Microsoft\Windows\CurrentVersion\Uninstall\${PRODUCT_NAME}"
!define PRODUCT_UNINST_ROOT_KEY "HKLM"
!define PRODUCT_DEFAULT_PATH "C:\eastmoney\EmTrade"
!define PRODUCT_EXE_NAME "Update.exe"
...
Function Use
   Exec '"$INSTDIR\EmTrade.exe" EmTrade'
   Call AlterButton
FunctionEnd
...
Section Main
  ...
  SetOutPath "$INSTDIR"
  SetOverwrite ON
  File "..\indep\Microsoft.VC80.CRT.manifest"
  File "..\indep\Microsoft.VC80.MFC.manifest"
  File "..\indep\mfc80u.dll"
  File "..\indep\msvcp80.dll"
  File "..\indep\msvcr80.dll"
  File "..\indep\TradeSocket.dll"
  File "..\indep\TradeMain.dll"
  File "..\indep\EmTrade.exe"
  File "..\indep\TradeUpdate.exe"
  File "..\indep\TradeUpdate.ucf"
  File "..\indep\TradeWebNet.dll"
  File "..\indep\TcpQuote.dll"
  File "..\indep\uninst.exe"

  SetOutPath "$INSTDIR\trade"
  SetOverwrite ON
  File /r "..\indep\trade\config"
 
  SetOutPath "$INSTDIR\trade"
  SetOverwrite ON
  File /r "..\indep\trade\data"
  
  SetOutPath "$INSTDIR\trade"
  SetOverwrite ON
  File /r "..\indep\trade\res"
...
;  #		CreateShortCut "$DESKTOP\${SHORTCUT_NAME}.lnk" "$INSTDIR\EmTrade.exe" "" "$INSTDIR\trade\res\dongcai.ico"
;  #${EndIf}
  
  ${If} $Checkbox2_State == 1
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\1_${PRODUCT_NAME}.lnk" "$INSTDIR\EmTrade.exe" "EmTrade" "$INSTDIR\trade\res\dongcai.ico"
...
;  	${if} $R0 >= 6.0
;	    SetOutPath "$INSTDIR"
;		#CreateShortCut "$INSTDIR\${SHORTCUT_NAME}.lnk" "$INSTDIR\EmTrade.exe"
;		#SetOutPath $WINDIR
;		ExecShell taskbarunpin "$DESKTOP\${SHORTCUT_NAME}.lnk"
;		Delete "$DESKTOP\${SHORTCUT_NAME}.lnk"
;		CreateShortCut "$DESKTOP\${SHORTCUT_NAME}.lnk" "$INSTDIR\EmTrade.exe" "EmTrade" "$INSTDIR\trade\res\dongcai.ico"
;		ExecShell taskbarpin "$DESKTOP\${SHORTCUT_NAME}.lnk"
;	 ${else}
;		  Delete "$DESKTOP\${SHORTCUT_NAME}.lnk"
;		  CreateShortCut "$DESKTOP\${SHORTCUT_NAME}.lnk" "$INSTDIR\EmTrade.exe"
;		  CreateShortCut "$QUICKLAUNCH\${PRODUCT_NAME}.lnk" "$INSTDIR\EmTrade.exe"
;	  ${EndIf}
;  ${EndIf}
  ...
  WriteRegStr HKLM "${PRODUCT_DIR_REGKEY}" "" "$INSTDIR\EmTrade.exe"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayName" "$(^Name)"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "UninstallString" "$INSTDIR\uninst.exe"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\EmTrade.exe"
...
Section Uninstall
  ...
  Delete "$INSTDIR\TradeSocket.dll"
  Delete "$INSTDIR\TradeMain.dll"
  Delete "$INSTDIR\EmTrade.exe"
  Delete "$INSTDIR\TradeUpdate.exe"
  Delete "$INSTDIR\TradeUpdate.ucf"
  Delete "$INSTDIR\TradeWebNet.dll"
  Delete "$INSTDIR\TcpQuote.dll"
  
  RMDIR /r "$INSTDIR\trade\config"
  RMDIR /r "$INSTDIR\trade\data"
  RMDIR /r "$INSTDIR\trade\res"
  RMDIR /r "$INSTDIR\trade"
;======================================================
